...
var { Dimensions, Text, View, TouchableOpacity, PanResponder, Animated, StyleSheet } = ReactNative;
...
var ViewPager = createReactClass({
	...
	movePage(step, gs, animate = true) {
		...
		if (animate) {
			this.fling = true;
			this.props.animation(this.state.scrollValue, scrollStep, gs).start(event => {
				if (event.finished) {
					postChange();
				}
